...
export default function RegistroPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (!inviteCode.trim()) {
      setError('El código de invitación es obligatorio para registrarse');
      return;
    }
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        setError('Este email ya está registrado. Inicia sesión en lugar de registrarte.');
      } else {
  ...
  const handleAppleSignUp = async () => {
    ...
    try {
      await signInWithApple(inviteCode.trim());
      router.push('/admin');
    } catch (err: any) {
      if (err.code === 'auth/email-already-in-use') {
        setError('Este email ya está registrado. Inicia sesión en lugar de registrarte.');
